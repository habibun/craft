...
class ProductHistoryController extends Controller
{
    ...
    private function _getProductInResults()
    {
        ...
        $query = $em->createQuery(
            'SELECT IDENTITY(pl.product), sum(pl.quantity) as total_product_in,pr.name as name
    ...
    private function _getProductOutResults()
    {
        $em = $this->getDoctrine()->getManager();
        $query = $em->createQuery(
            'SELECT IDENTITY(i.product), sum(i.quantity) as total_product_out,pro.name as name
